...
def train_classifier_simple(model, train_loader, val_loader, optimizer, device, num_epochs,
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        val_accuracy = calc_accuracy_loader(val_accs, model, device, num_batches=eval_iter)
...
if __name__ == '__main__':
    ...
    num_classes = 2
    model.out_head = nn.Linear(in_features=BASE_CONFIG["embedding_dim"], out_features=num_classes)
    model.to(device)
    ...
    end_time = time.time()
    execution_time_minutes = (end_time - start) / 60
